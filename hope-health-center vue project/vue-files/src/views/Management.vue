      ...
      <table class="patients-table">
        ...
        <tbody>
          <tr v-for="(patient, index) in filteredPatients" :key="patient.id">
            ...
            <td>
              <button @click="viewPatient(patient)" class="view-link">View</button>
              <button @click="deletePatient(patient.id, index)" class="delete-link">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...
    deletePatient(patientId, index) {
      if (confirm(`Are you sure you want to delete patient: ${this.filteredPatients[index].fullName || this.filteredPatients[index].name}?`)) {
